jQuery(document).ready(function($) {
    ...
    function initWysiwygEditors() {
        // Find all wysiwyg textareas that are not already initialized
        $('.wysiwyg').each(function(index) {
            const $textarea = $(this);
            const id = $textarea.attr('id');
            
            // If it already has an ID and TinyMCE is initialized for it, skip.
            // Also, assign a new unique ID if it doesn't have one or if it's a new instance.
            if (!id || !tinymce.get(id)) {
                ...
                tinymce.init({
                    selector: '#' + uniqueId,
                    menubar: false,
                    toolbar: 'formatselect | bold italic underline | forecolor backcolor | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link unlink | undo redo',
                    plugins: 'lists link',
                    branding: false,
                    setup: function(editor) {
                        editor.on('change', function () {
                            editor.save(); // push content to <textarea>
                            // Trigger jQuery change on the textarea itself, which the general handler will pick up
                            $('#' + editor.id).trigger('change'); 
                        });
                        // Also trigger update on init for existing content
